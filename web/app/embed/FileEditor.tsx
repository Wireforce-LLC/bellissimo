...
import { Editor, Monaco, MonacoDiffEditor, useMonaco } from "@monaco-editor/react";
...
<path
  ...
  if (file.is_file && (file.path.endsWith(".html") || file.path.endsWith(".htm"))) {
    return  <svg
    viewBox="0 0 16 16"
    fill="#777BB3"
    className="size-3"
  ...
  if (file.path.endsWith("objects")) {
    return <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="size-3 fill-lime-500">
  ...
  if (file.path.endsWith("plugins")) {
    return <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="currentColor" className="size-3 fill-orange-500">
...
function File({
  ...
  return (
    <div
      ...
      className={
        classNames("w-full hover:bg-gray-100 font-mono cursor-pointer flex flex-row items-center gap-2 bg-white py-1 px-2", {
  ...
  return (
    <div className="w-full h-full bg-white overflow-hidden flex flex-row">
              ...
              setCursorOnFile={(it) => {
                if (!safeOut) {
                  toast(<div>
                    ...
                    <div className="text-xs text-gray-500">You have unsaved changes</div>
      ...
      <div className="overflow-hidden w-full h-full" ref={editorRef}>
        <div className="w-full overflow-x-auto border-b border-gray-200 flex flex-row items-start justify-start gap-2 bg-white py-1 px-2">
          {tabs?.map((it) => (
            <div onClick={() => {
              ...
              <Tab isActive={cursorOnFile == it.path} key={it.path} name={it.name} path={it.path} />
            </div>
          ))}
        </div>
        ...
        {/* Editor */}
        {!cursorOnFile && <div className="w-full h-[70%] flex flex-col items-center justify-center">
          <img src={manWithIdea} alt="" className="w-64" />
          <h1 className="font-medium text-gray-black">Before your idea become real</h1>
          <p className="text-xs text-gray-500">Select a file, or create a new one</p>
        </div>}
