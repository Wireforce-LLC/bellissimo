  ...
  return <>
    <nav
      className={classNames(
        "w-full bg-white border-b border-b-gray-100 h-[40px] md:h-[46px]",
        className
      )}
    >
            ...
            <li>
              ...
              <HLink
                ...
                className={classNames("cursor-pointer px-2 py-2 text-xs", {
                  "text-gray-400 hover:text-black font-normal": !item.isActive,
      ...
      <div className="mx-auto px-4 container md:hidden flex items-center justify-between h-full">
        <div className="py-2 pt-2.5 flex flex-row space-x-2 select-none mr-4">
          ...
          <span className="-mt-0.5 text-sm font-semibold">Paper Analytics</span>
        </div>

        <button onClick={() => setMobileMenuOpen(!isMobileMenuOpen)} className="cursor-pointer">
          ...
          <ul className="absolute bg-white z-10 w-full border-b border-b-gray-200 drop-shadow-sm">
            {
              items?.flatMap((item) => (
                <li
                  className="px-3 py-2 hover:bg-gray-50 focus:bg-gray-50 cursor-pointer font-medium text-sm"
                >
                  {/* eslint-disable-next-line jsx-a11y/role-supports-aria-props,jsx-a11y/no-redundant-roles */}
                  <a
                    href={item.href}
