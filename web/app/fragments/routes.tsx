...
export default function Routes() {
  ...
  const [modalEditResourceId, setModalEditResourceId] = useState<string|undefined>();
  const [modalEditFilterId, setModalEditFilterId] = useState<string|undefined>();
  ...
  return (
    <>
      ...
      <EazyModal title="Edit resource" isVisible={!!modalEditResourceId} intent={() => setModalEditResourceId(undefined)}>
        <EditResourceEmbed resourceId={modalEditResourceId}/>
      </EazyModal>

      <EazyModal title="Edit filter" isVisible={!!modalEditFilterId} intent={() => setModalEditFilterId(undefined)}>
      ...
      <Table2
        ...
        valueTransformer={{
          filter_id(value, row) {
            return (
              <span className="flex flex-row gap-1.5 items-center text-zinc-700">
                ...
                <span data-filter-id={row.filter_id} onClick={() => setModalEditFilterId(row.filter_id)} className="text-zinc-400 hover:text-indigo-500 underline decoration-dotted font-medium">
                {value}
                </span>
              </span>
            );
          ...
          params(params, row) {
            return <span className="text-zinc-400 hover:text-indigo-500 underline decoration-dotted font-medium" onClick={() => setModalOverviewData(row.name)}>
              {
          ...
          resource_id(value, row) {
            return <span className="text-zinc-400 hover:text-indigo-500 underline decoration-dotted font-medium" onClick={() => setModalEditResourceId(row.resource_id)}>{value}</span>;
          },
